...
public class Playground {
	...
	public void spielfeldLeeren() {
		Arrays.fill(spielfeld[0], freiesFeld);		// zeile 0 wird mit freiesFeld-Symbol �berschrieben/ gef�llt
		Arrays.fill(spielfeld[1], freiesFeld);		// zeile 1 wird mit freiesFeld-Symbol �berschrieben/ gef�llt
		Arrays.fill(spielfeld[2], freiesFeld);		// zeile 2 wird mit freiesFeld-Symbol �berschrieben/ gef�llt
	}
	
	public boolean setzeSymbol(String symbol, String feld) {
		switch(feld) {		// die zahlen zB. 0, 0 sind die 2d-Array koordinaten.
		case "q1": if(isAble(0, 0, symbol)) { return true; } break;		// isAble kontrolliert ob das Feld zul�sst das man setzen kann. wenn gesetzt werden konnte, wird true zur�ckgegeben
		case "q2": if(isAble(1, 0, symbol)) {  return true; } break;	// und das programm gibt dann ebenfalls true zur�ck.
		...
		return false;	// wenn isAble false zur�ckgibt, wird false zur�ckgegeben.
	}
	
	private boolean isAble(int row, int column, String symbol) {
		if(spielfeld[row][column].equals(freiesFeld)) {		// kontrolliert ob an der �bergebenen parameter zeile und spalte des 2d arrays ein freiesFeld ist. Also ob man setzen kann.
			spielfeld[row][column] = symbol;		// wenn gesetzt werden kann, wird symbol in die koordinate gespeichert.
			return true;		// und anschlie�end ein true zur�ckgegeben.
		}
		return false;		// wenn nicht gesetzt werden kann, wird ein false zur�ckgegeben.
	}
	
	public int winnable() {
		// links nach rechts und oben nach unten pr�fen.
	...
	public int sonderFaelle() { // es gibt eine methode die schnell zum sieg f�hrt und die CPU schnell austricksen l�sst. Diese wurde hier behoben. So das die Schwierigkeit sehr schwer ist.
		// wenn feld 1 und feld 9 zB. gleich sind und dann auf feld 7 setzt, hat man 2 m�glichkeiten zu gewinnen. man hat aber nur einen zug um zu blocken. diesen zug erkennt die CPU vorher
		// und blockt das.
		...
		// wenn zB. speiler auf feld 4 setzt, anschlie�end auf feld 8, kann er n�chsten zug auf feld 7 setzen (also dazwischen) um 2 gewinnm�glichkeiten zu haben. dieser wird hier geblockt.
	...
	public boolean wichtigeFelder(String symbol) {
		// wichtige felder um dem Spieler schwierig zu machen, sind vorallem das mittlere Feld, anschlie�end die Seite.
	...
	public String getWinner() {
		...
		return freiesFeld;		// wenn keiner gewonnen hat, wird freiesFeld-Symbol zur�ckgegeben. also ein (*) symbol.
	}
	...
	public String toString() {
		...
		for (int zeile = 0; zeile < spielfeld.length; zeile++) {		// for-schleife geht die zeilen.
			zwischenspeicher = zwischenspeicher + "\n\n\n" + (zeile + 1) + "\t";	// zeile+1 f�r zahlen links neben dem feld.
		...
		zwischenspeicher = zwischenspeicher + "\n\n";
		return zwischenspeicher;		// als fertiges String zur�ckgegeben.
	}
}